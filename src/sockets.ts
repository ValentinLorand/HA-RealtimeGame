...
export function manageInGameMessage(action: string, secret: string, gameworld: GameWorld): Record<string, unknown> {
  const player = gameworld.getPlayerFromSecret(secret);
  //If no player found => ERROR
  if (player === undefined) {
    log.warning("Unknown player secret : " + secret);
    throw new Error('Bad secret given');
  }
...
export function manageOutGameMessage(action: string, name: string, secret: string, ws: WebSocketClient, gameworld: GameWorld): Record<string, unknown> {
